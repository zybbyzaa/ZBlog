...
const Article = {
    ...
    getArticles(pageNum, keyword) {
        const sort = {
            update_time: -1
        }
        const condition = keyword === '' ? {} : {title: new RegExp(keyword, 'i')}

        return ArticleModel.find(condition).sort(sort).populate({path: 'author', select: 'username avatar'}).limit(3).skip((pageNum - 1) * 3).exec()
    },
    getArticleById(id) {
        const query = {
            _id: id
